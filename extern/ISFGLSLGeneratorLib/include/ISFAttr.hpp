...
class ISFAttr	{
	protected:
		...
		ISFVal				_currentVal = CreateISFValNull();
		ISFVal				_minVal = CreateISFValNull();	//	if it's an audio/audiofft, it's a long-type val.  otherwise, null or an ISFVal subclass of the appropriate type
		ISFVal				_maxVal = CreateISFValNull();	//	if it's an audio/audiofft, it's a long-type val.  otherwise, null or an ISFVal subclass of the appropriate type
		ISFVal				_defaultVal = CreateISFValNull();
		ISFVal				_identityVal = CreateISFValNull();
	...
	public:
		...
		*/
		ISFAttr(const std::string & inName,
			...
			const ISFVal & inMinVal=CreateISFValNull(),
			const ISFVal & inMaxVal=CreateISFValNull(),
			const ISFVal & inDefVal=CreateISFValNull(),
			const ISFVal & inIdenVal=CreateISFValNull(),
